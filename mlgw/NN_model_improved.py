...
#FIX THIS ISSUE!
if '/home/tim.grimbergen/new_MLGW/MLGW-master/dev/mlgw_v2' in sys.path:
	sys.path.remove('/home/tim.grimbergen/new_MLGW/MLGW-master/dev/mlgw_v2')
if '/home/tim.grimbergen/new_MLGW/MLGW-master/dev/mlgw_NN/code' in sys.path:
	sys.path.remove('/home/tim.grimbergen/new_MLGW/MLGW-master/dev/mlgw_NN/code')
...
class mlgw_NN(keras.Sequential): #maybe this becomes too expensive anyway? adding the features so many times...
	...
	def fit(self, x = None, y = None, epochs = None, validation_data=None, batch_size=None, callbacks=None):
        ...
        #if x.shape[-1] ==3:
            #augmentfeatures
		if validation_data: validation_data = (augment_features_general(validation_data[0], feature_list=self.features), validation_data[1]) 
	...
	def predict(self, X_data, **kwargs):
		X_data = augment_features_general(X_data, feature_list=self.features)
...
def build_model_full(output_nodes, hyperparameters, loss_weights=None):
    
	def build_model(hp):
        
		if isinstance(hyperparameters["units"], tuple):
			units = hp.Choice('units', list(hyperparameters["units"]))
		else:
			units = hp.Fixed('units', hyperparameters["units"])
		if isinstance(hyperparameters["layers"],tuple):
			layers = hp.Choice('layers', list(hyperparameters["layers"]))
		else:
			layers = hp.Fixed('layers', hyperparameters["layers"])
		if isinstance(hyperparameters["activation"], tuple):
			activation = hp.Choice('activation', list(hyperparameters["activation"]))
		else:
			activation = hp.Fixed('activation', hyperparameters["activation"])
		if isinstance(hyperparameters["learning_rate"],tuple):
			lr = hp.Choice('learning rate',list(hyperparameters["learning_rate"]))
		else:
			lr = hp.Fixed('learning rate',hyperparameters["learning_rate"])
		if isinstance(hyperparameters["feature_order"], tuple):
			feature_order = hp.Choice('feature order', list(hyperparameters["feature_order"]))
		else:
			feature_order = hp.Fixed('feature order', hyperparameters["feature_order"])
		...
		true_weights = [1]*output_nodes if not isinstance(loss_weights,list) else loss_weights
		
		model.compile(loss=LossFunctions('custom_mse',weights=true_weights).LF,
...
def tune_model(out_folder,name, quantity, PCA_data_loc, PC_to_fit, hyperparameters, max_epochs=1000, init_trials=None, trials = 10):
	...
	true_weights = np.sqrt(np.array(PCA_data.pca.PCA_params[2]))[PCA_data.PC_comp] 
	true_weights = true_weights / min(true_weights)
	
	build_model = build_model_full(PCA_data.train_var.shape[1],
									hyperparameters,
									loss_weights = true_weights)
...
def analyse_tuner_results(file_loc, save_loc=None):
	...
	for x in os.listdir(file_loc):
		if x[:5] == 'trial':
			with open(file_loc+'/'+x+"/trial.json", "r") as f:
...
=======
	...
	true_weights = np.sqrt(np.array(PCA_data.pca.PCA_params[2]))[PCA_data.PC_comp]
	true_weights = true_weights / min(true_weights)
	print("Weights used are: ", true_weights)
...
def create_residual_PCA(data_loc, model_loc, save_loc, quantity, components, savefigs=True):
	#load in the PCA dataset
	data = PcaData(data_loc, components, quantity)
