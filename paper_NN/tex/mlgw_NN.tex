...
We set the starting frequency in the range $[10, 20]Hz$ \tim{By what procedure do we set this starting frequency? I know I can look it up at the GitHub code, but is this something worthy to write down here?}.
...
While the model is already applicable for most of the parameter estimation problems with current detectors, it is desirable to increase its range of validatity of our model, both in parameter space and in time.
...
\tim{Maybe we could add a paragraph on different neural network topologies (not the same number of units for each hidden layer) and repeat what we said about improving HMs by using separate tuning?}
