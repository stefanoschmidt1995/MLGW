...
\BS{[suggestion, please improve/correct/fix]\\
...
waveform's amplitude and phase in a lower
dimensional space using a principal component analysis.
We train \texttt{mlgw} on about $\mathcal{O}(10^3)$ \texttt{TEOBResumS} effective-one-body
waveforms with mass ratios $q\in[1,128]$ and aligned dimensionless
spins $\chi\in[-0.99,0.99]$. We obtain a model that is faithful to the
training set at the ${\sim}10^{-3}$ average level, while speeding up
the single waveform generation of a factor 10-50 (depending on the
binary mass/initial frequency.)
...
Future applications comprise the analysis of waveform systematics and
