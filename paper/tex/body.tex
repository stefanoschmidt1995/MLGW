...
By contrast, an EOB model, informed by NR simulations and test-mass regime, does not 
have such drawback since it is designed to generate waveforms all over the BBH parameter space. 
...
they are also typically able to robustly generate waveforms 
...
As a relevant physical application, we use the our ML model \texttt{mlgw}, trained on \texttt{TEOBResumS} model, 
...
slightly larger than the published LVC ones obtained using different waveform models.
\sschmidt{I removed the 1/q column so the table would fit in a single column. Is it ok? The old table is commented, just in case...}
} 
