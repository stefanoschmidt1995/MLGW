...
\par
In this work, we explore this opportunity. We developed a ML model that is able to generate a GW signal from a binary black hole (BBH) coalescence. We demonstrate that our model can generate GW signals significantly faster than state-of-the-art EOB models while keeping the same accuracy. Moreover, we also present a successful application in a parameter estimation setting, by computing posterior distributions for GW150914.
\par
Our fast model can be effectively applied to several open problems in the state-of-the-art research.
First of all, being faster (\textbf{that's strong! You should be very sure of it}) than every waveform model, it could be used to speed up the search of GW waves and the subsequent analysis.
...
%Parameters $(m_1,m_2, s_1, s_2, d_L, \iota, \phi_0)$ are randomly chosen in the set $\tilde{\mathcal{P}} = [5.,100.]\times[5.,100.]\times[-0.8,0.95]\times[-0.8,0.95]\times[0.5,10.]\times[0,\pi)\times[0,2\pi)$.
}
	\label{fig:F_hist}
\end{figure}
%%%%
...
As long as the $s_1$ dependence is considered, the most striking feature is the inverse correlation of mse and mismatch. \textbf{WHY IS IT SO?? IT'S REALLY WEIRD... IS THERE SOMETHING WRONG?? OR IT CAN BE EXPLAINED ON PHYSICAL BASIS? ANYWAY, FOR SEOB WE HAD A COMPLETELY DIFFERENT RESULT. WHY?}
\par
...
Whean deling in a real detection scenario, we are often interested in generating a WF which starts from a given frequency $f_{min}$. Such fequency is usually set by the detector's sensitivity window.
Thus, it is crucial to measure the speed up that our model can provide in performing such task.
We define the speed up $\mathcal{S}$ as the ratio between the runtime of \texttt{TEOBResumS} and the runtime of \texttt{mlgw} to produce the a waveform starting from a given $f_{min}$. Each waveform is produced with constant total mass $M = 100 M_{\odot}$ and random parameters; the WF is sampled at $f_{sam} = \SI{2048}{Hz}$.
We consider the two cases with $f_{min} = \SI{5}{Hz}$ and $f_{min} = \SI{20}{Hz}$; the first choice refers to the hypotetical lower bound for the sentivity of the Einstein telescope (ET), while the second is close to the Advanced-Ligo lower sensitivity.
In figure~\ref{fig:time_performance_hist} we report the histogram of the measured speed up values for the two situations.
...
\begin{figure}
	...
	\caption{
Histogram for values of the speed up given by \texttt{mlgw}, as compared with \texttt{TEOBResumS} model.
It is computed on $N=2000$ test waveforms. Each WF is generated with random physical parameters and has a minimum frequency of $\SI{5}{Hz}$ (top panel) and $\SI{20}{Hz}$ (top panel).
...
\par
\textbf{Comparison with ROM!!! Will you win??}
\\
also: should you remove the inset with the cumulative distribution and make the caption bigger??
\par
It is interesting to have a knowledge of the time taken by each stage of the WF generation procedure.
...
Our model shows excellent agreement with the underlying training set. At test times the median mismatch is $\bar{\mathcal{F}}\sim 5 \cdot 10^{-3}$. Lastly, the generation time from \texttt{mlgw} is smaller than the underlying training model by a factor of $\sim 35$, for a waveform starting at a frequency of $\SI{5}{Hz}$.
\textbf{Say something about ROM (if there is anything to say...)}.
\par
...
\par
Our model opens up interesting opportunities in GW data analysis.
As shown in fig. \ref{fig:time_performance_hist}, the model performs is the most useful whenever a long waveform is computed. Furthermore, the waveform generation time does not depend on the complexity of the underlying surrogate model. The training WFs can be computed with high accuracy, also with a high computational cost, without affecting the performance of the WF generation.
\par
This fact is crucial for detection of low frequency signals, as is the case for ET. The analysis of such signals can be performed in the same time required to deal with shorter signals: it will become feasible, even with a small amount of resourches.
...
\par
As already noted, a parameter estimation with mlgw is remarkably fast. This opens up the opportunity for an analysis of the catalog of all the observed GW events. There are several advantages.... \textbf{See email for details...}
A second paper, in publication, will be devoted to such analysis.
\par
\textbf{Is there more to say on that??}\\
\par
