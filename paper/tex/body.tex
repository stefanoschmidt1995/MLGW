...
By contrast, and EOB model, informed by NR simulations, does not have such drawback as it can generate 
...
%======================
%
The goal of the present work is to provide an accurate Machine Learning model which outputs the functions $A(\boldsymbol{\vartheta})$ and $\phi(\boldsymbol{\vartheta})$ (eq.~\eqref{eq:h_ML}), as generated by the state-of-the-art time domain WF models.
Because of scale invariance of GR, the dependence on total mass $M= m_1+m_2$ can be inserted analytically and we only need to consider the variables $\tilde{\boldsymbol{\vartheta}} = (q, s_1, s_2)$, where $q= m_1/m_2 \geq 1$.
More formally, we seek a ML model that reliably reproduce the following map:
\begin{align}
	(q, s_1, s_2) &\longmapsto A_{(q, s_1, s_2)}(t) \label{eq:objective_amp}\\
	(q, s_1, s_2) &\longmapsto \phi_{(q, s_1, s_2)}(t) \label{eq:objective_ph}
\end{align}
\par
In the context of ML, our task reduces to performing a regression\footnote{A regression is a statistical method to infer the relationship between a set of ``independent variables" and a set of ``dependent variable". A model consist in a functional form for such relation, usually with many free parameters to be specified. By looking at the data, one should be able to make a proper choice for the paramters. This is common in almost every ML model.} 
from the orbital paramters to the WF. 
Before being able to perform a regression, several steps are required.

\begin{enumerate}[label=(\Alph*)]
	\item \textit{Setting a time grid}. Each WF must be represented on a discrete time grid, which allows for efficient and reliable reconstruction on an arbitrary grid. After this operation, the functions $A(t)$ and $\phi(t)$ are represented as vectors\footnote{
...
\par
The value of $f$ at an arbitrary time must be found by interpolation and this introduces an error in the value of the function.
...
As can be seen in fig.~\ref{fig:tau_grid}, such choice ensures that more points are accumulated around the amplitude peak.
\par
...
To generate the training waves the \texttt{TEOBResumS} model is used \sschmidt{(CITE...)}.
...
\begin{align}
	\bar{\mathcal{F}}[h_1,h_2] &\equiv \argmin_{\phi_0} \left[ 1- \frac{\langle h_1, h_2 e^{i\phi_0} \rangle}{\sqrt{\langle h_1, h_1 \rangle \langle h_2, h_2 \rangle}} \right]	\label{eq:mismatch_def} \\
...
\section{Parameter estimation on GWTC-1 catalog}
\sschmidt{Choose a better title and write the whole section...}
\par
