...
\par
In this work, we explore the application of ML to WF generation and we build a ML model that allows to reproduce waveforms from any EOB model for BBH coalescence. We demonstrate that such ML-based model can generate 
...
At the same time it shows a good agreement with the train model, at the $10^{-3}$ level when averaged on the whole parameter space.
For simplicity, we focus here only on the 
dominant $\ell=m=2$ quadrupole waveform and we only consider the align spin case.

EOB models are the ideal candidate for training our ML model: they are moderatly fast to run and, although they are {\it checked}
(i.e. NR-informed) only on the part of the parameter space covered by NR simulations, 
they are also typically able to robustly generate waveforms in a large set
outside the NR-information domain (e.g. large mass ratios and
spins).
This is the case of the most recent avatar of 
...
We use both \texttt{TEOBResumS\_SM} and \texttt{SEOBNRv2}/\texttt{v4} to train different versions of the model.
\par
\iffalse
...
%\footnote{Let us note, in passing, that the ML framework 
...
\fi
As a relevant physical application, we use the our model \texttt{mlgw}
...
To asses any difference between \texttt{TEOBResumS} and \texttt{SEOBNRv4}, we perform two separate analysis of the catalog, using two different version of \texttt{mlgw}, trained on the two EOB models.
The analysis does not show large disagreement between the two versions of \texttt{mlgw}.
Furthermore, the physical parameters we find are 
...
Since the dependence on the two angles, on the distance and total mass is known, we fix their value to ${\iota = \varphi_0 = 0}$, ${d_L = \SI{1}{Mpc}}$ and $M = \SI{20}{M_\odot}$ and we only work with waveforms ${h_{FIT}(t; \boldsymbol{\vartheta}) = h(t; d_L = \SI{1}{Mpc}, \iota = \varphi_0 = 0, M = \SI{20}{M_\odot}, \boldsymbol{\tilde{\vartheta}}) }$:
\begin{align}  
...
\footnote{
In the figure, the histogram with $f_{min} = \SI{20}{Hz}$ (bottom), displays two peaks.
This can be understood as follows: the WFs with high positive spins tends to be longer (at constant $f_{min}$), thus requiring more execution time for \texttt{TEOBResumS}; on the other hand in such cases \texttt{mlgw} requires approximately a constant time for the generation of such WFs. Then the two peaks measure indirectly the execution time of \texttt{TEOBResumS}.}.
...
\end{figure*}
Table~\ref{tab:summary} summarize the results with {\tt TEOBResumS} and with {\tt SEOBNRv4}.
...
We started this program by analysing GWTC-1 with \texttt{mlgw} trained on \texttt{TEOBResumS} and \texttt{SEOBNRv4} and highlited some differences in the predictions as compared to the published results (see section \ref{sec:GWTC1}). Future work might repeat such an analysis on other EOB models or with more observations.
\par
...
\par
In conclusion, we presented \texttt{mlgw}, an off-the-shelf ML model for gravitational waves signals from BBHs. We demostrated that \texttt{mlgw} is fast, accurate and easy to train and to use. We anticipate that \texttt{mlgw} will enable studies hitherto unfeasible due to the lack of fast and easy to use models.
