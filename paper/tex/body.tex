...
distribution $\mathcal{F}_m = 5.5 \cdot {10^{-4}}$. Such results are similar to the discrepancies 
...
\begin{figure}[t]
    ...
    \begin{minipage}{\linewidth}
		...
		\end{minipage}
    \end{minipage}
	\caption{Test mismatch and mean squared errors (MSE) for the first PC of the phase, as a function of the 
	orbital parameters $\vartheta = (q, M, s_1, s_2)$. The histograms hold $145061$ waveforms, with randomly 
	drawn parameters. In the left column, the color mesh refers to test mismatch; in the right column, we report 
	test MSE on the first PC. Each physical quantity $(M,s_1,s_2,q)$ is discretized in $35$ bins. Each wave 
	starts $\SI{8}{s}$ before merger.
}
	\label{fig:countour}
\end{figure}
}
...
in the case of $q$~\footnote{See~\cite{Ohme2013PCA_GW} for a closely related discussion on PCA components 
and its dependence on physical parameter.}. In the third row of Fig.~\ref{fig:countour}, the inverse correlation between 
