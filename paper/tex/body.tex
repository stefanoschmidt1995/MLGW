...
In the context of ML, our task reduces to performing two regressions from $\tilde{\boldsymbol{\vartheta}}$ to the amplitude and phase of WF. 
...
As the phase has a rather regular behavior, it is not important to tune the time grid on it. For this reason, a single grid for amplitude and phase, tuned on the behavior of amplitude, is used.
...
The choice of working with the variable $\log q$ rather than $q$ is based on validation results (not reported here). Heuristically, it prevents the values of the data features from varying too much within the range of interest, thus yielding more stable numerical performance.
...
We then vary $N_{\rm grid}$ and $\alpha$ and report the resulting mismatch $\mathcal{F}[\mathbf{f}_{EOB}, \mathbf{f}_{N_{\rm grid}, \alpha}]$ in Fig.~\ref{fig:N_grid}.
%%
\begin{figure}[!t]
	\centering
...
not generalize the data (underfitting). In the choice of number of training points, the comparison between train and 
...
first 3 PCs as a function of the number of training points. Data refers to a MoE model fitted for 4 PCs of the phase dataset. 
The models has ${N_{\rm exp} = 4}$ and performs basis function expansion with a 4th degree polynomial.
\begin{figure}
	\centering
...
We report our results in the histogram in Fig.~\ref{fig:F_hist}. We report a median value of the 
distribution ${\mathcal{F}_m = 5.5 \times {10^{-4}}}$. Such results are similar to the discrepancies 
...
The latter is useful to test the accuracy of the fit before wave reconstruction. The results are reported 
...
\begin{figure}
	...
	\begin{minipage}{\factor\linewidth}
	    \includegraphics[width=\linewidth]{compare_time_hist_5}
	\end{minipage}\hfill
	\begin{minipage}{\factor\linewidth}
	    \includegraphics[width=\linewidth]{compare_time_hist_20}
	\end{minipage}
...
equivalent from this point of view, so that we simply refer to the model as {\tt mlgw} here 
and below.
...
the median mismatch is $\bar{\mathcal{F}}\sim 5 \times 10^{-4}$.
...
training: as discussed above, at least $O(5\times 10^3)$ waveforms are needed.
