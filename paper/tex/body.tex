...
A Machine Learning model is a promising alternative to the
state-of-the-art waveforms generators.
%
\BS{[would say] to ACCELERATE waveform generation}
%
...
redundant
%
\BS{[so why you use them all hundreds?]}) 
%
...
%
\BS{[But searches don't use template banks pre-computed?]}
%
...
For this reason, we approximate the full multipolar waveform with just the single complex quantity $H_{22}$ and we define
...
\end{equation}
%
\BS{[what means the tilde on top of h? Usually this is used for the
    FT. If you mean an approximation I'd use: $h\simeq...$][Does the
    factor 4 takes into account the c.c.?]}
%
and we express $\tilde{h}$ in terms of its amplitude and phase
(\BS{note we adopt a nonstandard sign convention for the phase}):
...
\BS{[alt.] Since BBH waveforms trivially scales with the binary mass
  $M$, we only need to consider the variables
  $\tilde{\boldsymbol{\vartheta}} = (q, s_1, s_2)$, where $q= m_1/m_2
  \geq 1$, to describe the waveform's multipoles.}
...
\end{align}
\sschmidt{Cited here the paper on scaling relations, beacause we used
  it and was really helpful. Keep it?}\BS{[ma anche no]}
...
%
\BS{[orbital parameters have been defined above as (m1,m2,s1,s2), but now you refer to (q,s1,s2)]}
%
...
In ML jargon, this procedure is called preprocessing and aims to create a standard representation for all the data available (in our case the WFs).
	...
	\item \textit{Reducing the dimensionality of a WF}. In order
          to make the regression feasible, \oldnewtxt{we need to reduce the dimensionality of a WF: we must be able to represent a single WF with very few real numbers; that is, we build a low dimensional representation of the WF}{we build a low dimensional representation of the WF}. This operation must be invertible: once a low dimensional representation is given, one should be able to reconstruct the higher dimensional WF.
	\item \textit{Learning a regression}. \oldnewtxt{Eventually, w}{W}e train a model to perform the regression from $(q, s_1, s_2)$ to the low dimensional representation of the WF.
\end{enumerate}
\BS{We discuss these points in detail in what follows.}
...
\sschmidt{Do we really need this??}\BS{[I think no, but up to you]}
}
	\label{fig:tau_grid}
\end{figure}
%
...
To make the interpolation effective, \oldnewtxt{we want a finer grid
  when the function changes much.}{we introduce a grid adapted to the
...
\par
The \oldnewtxt{lenght}{length} of the time grid determines the maximum lenght of the WFs that the model can generate.
...
$\bar{t}$.\BS{[yes, but what you choose?]}

\BS{[Here it is important to state the ranges of mass ratio and spins
...
A theorem\BS{[what theorem?]} guarantees that, for zero mean data, the generators of subspace are the (orthonormal) first $K$ eigenvectors of the empirical covariance matrix $\Sigma \in \mathbf{Mat}(D,D)$. The eigenvectors are also called Principal Components (PCs) of the data.
...
%%%% Where do I put it??
\BS{[this goes at the end of conclusions]}
...
package provides also the gradients $\frac{\partial h}{\partial
  \vartheta_i}$ of the waveform. \BS{[How are these gradients calculated?]}
...
\end{align}
\BS{[define the Wiener product in the above eq.]}
We always use a flat noise curve (i.e. constant power spectral density for the detector noise).
%%%%%%%

\BS{[some colors in these figures would be nice and help the reader.]}
...
and they\BS{one ?} should choose the number of PCs, while checking MoE performance.
\par
...
%
\BS{[Isn't the EOBsurrogate the
best? and how about NRsurrogate?]}
...
\texttt{SEOBNRv4\_ROM}.
%
\BS{well, this depend on how you code in C and how much build-in
  features of python you use. The latter might accelerate things
  significantly (e.g. vectorization, GPUs)
  and be trivial to implement in python (something more difficult to
  achieve for the average physicist working in C)}
%
...
%Our model was built for the case of aligned spins and returns a wave given the BH's masses $m_1, m_2$ and (dimensionless) spins $s_1, s_2$ taking into account the correct dependence on source luminosity distance $d_L$, inclination angle $\iota$ and reference phase $\varphi_0$.
