"Little helper to compare outputs from lal and from precession package. Final conclusion: the two methods kind of agree (but not too much). How does precession performs with the WF?"
...
t_grid, alpha, beta = get_alpha_beta_L0frame(m1+m2, m1/m2, theta[2:5],theta[5:8], None, 400.)


t_grid_lal, alpha_lal, beta_lal = lalintegrate_PNeqs.get_alpha_beta(theta[0]/theta[1], theta[2:5],theta[5:8], 400., times = None, 
		t_shift = -0.,
		f_merger =None, #g.get_merger_frequency(theta_modes)
		)
