...
def twist_modes(h_22, h_21, alpha, beta, gamma):
	l = 2
	...
	D_mprimem = g._GW_generator__get_Wigner_D_matrix(l,[lm[1] for lm in mprime_modes_list], [lm[1] for lm in m_modes_list], gamma, -beta, -alpha) #(N,D,M'',M)

	#D_mprimem = np.conj(D_mprimem) #(N,D,M'',M) #complex conjugate (it does not seem to matter much)
	#print("function ",l, [lm[1] for lm in mprime_modes_list], [lm[1] for lm in m_modes_list])
	#print(D_mprimem.shape)
	#np.save('data/function_Wigner.npy',D_mprimem)
...
h_T_mlgw = twist_modes(h_TEOB_22, h_TEOB_21, alpha, beta, gamma)
...
h_T_mlgw_function = twist_modes(h_T_mlgw_function[0,:,0], h_T_mlgw_function[0,:,1], alpha, beta, gamma)
