...
params, alpha, beta_avg, beta_amp, beta_ph, times = load_dataset("temp.dat", N_data = None, n_params = 6, N_entries = 4)

plt.figure()
plt.title("avg")
plt.plot(times,beta_avg.T)
#plt.plot(times,(beta_avg+beta_amp*np.cos( beta_ph)).T)
plt.figure()
plt.title("ph")
plt.plot(times,beta_ph.T)
plt.figure()
plt.title("amp")
plt.plot(times,beta_amp.T)
plt.figure()
plt.title("alpha")
