...
if __name__ == '__main__':
    ...
    a = 0
    b = 100
    n = 99  # there are n + 1 grid points (0,..., n)
    ...
    new_grid = linspace(a, b, 500000)


    start_time = time.process_time_ns()/1e6 #ms

    c = cal_coefs(a, b, y)
    f_new = array([interpolate(x, a, b, c) for x in new_grid])

    end_time = time.process_time_ns()/1e6 #ms

    print("Number of points: ", len(new_grid))
    print("sline cython", end_time-start_time)

    from matplotlib import pyplot as plt
    line_actual = plt.plot(new_grid, f(new_grid), label='actual')
    line_approx = plt.plot(new_grid, f_new, '-.', label='interpolated')
